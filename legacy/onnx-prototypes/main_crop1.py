"""
Climbing AI — main_crop1: Emergency Zoom Fallback
==================================================
Adds emergency zoom fallback to main2.py: when a tracked climber is
not detected for LOST_FRAMES_TRIGGER frames, crops around their last
known position and retries detection there.

Usage:
    python main_crop1.py --source "your_video.MOV" --weight 45
    python main_crop1.py                               # live camera
"""
...
TRACK_MODEL_PT = "yolo11n.pt"
POSE_MODEL_PT  = "yolo11n-pose.pt"
...
ZOOM_CROP_HALF_W     = 300   # half-width of the zoom crop in pixels
ZOOM_CROP_HALF_H     = 400   # half-height of the zoom crop in pixels
...
# Distinct path colors for multi-climber — highly visible, all different
PATH_COLORS = [
    (0,   255, 255),   # bright yellow
    (255,  50, 255),   # magenta
    (50,  255,  50),   # bright green
    (255, 100,  50),   # orange-blue
    (50,  100, 255),   # red-orange
    (255, 255,  50),   # cyan
    (50,  255, 200),   # lime
    (200,  50, 255),   # pink-purple
]
# Maps pid → assigned path color index (assigned in order of first appearance)
_path_color_map  = {}
_path_color_next = [0]  # mutable counter
...
class PathTracker:
    ...
    def draw(self, frame, pid, color=None):
        ...
        if pts:
            cv2.circle(frame, pts[-1], 6, col, -1, cv2.LINE_AA)
...
class ClimbScorer:
    """Live score (0-100). Technical (60%): triangle, arm, force. Performance (40%): efficiency."""
...
class EmergencyZoomTracker:
    """
    Tracks missing frames per climber ID. After LOST_FRAMES_TRIGGER frames,
    crops around their last known position and retries detection there.
    Runs after full-frame detection as a fallback layer.
    """
...
def run(source, body_weight_kg=None, output_path=None):
    ...
    while True:
        ...
        # This prevents ghost IDs from accumulating regardless of proximity.
        # Uses a generous distance threshold — any new ID within 400px of a
        # lost known climber gets remapped back to the original ID.
        REMAP_DIST  = 400   # px — generous to handle fast movement
