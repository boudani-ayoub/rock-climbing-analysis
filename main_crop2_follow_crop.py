"""
Climbing AI — main_crop2: Follow Zoom for Tall Walls
=====================================================
Two-phase detection:
  Phase 1 — full-frame detection to find new climbers.
  Phase 2 — follow crop tracks each known climber as they climb,
             keeping them large in the crop regardless of wall height.

Usage:
    python main_crop2_follow_crop.py --source "your_video.MOV" --weight 45
    python main_crop2_follow_crop.py                           # live camera
"""
...
class FollowZoomTracker:
    """
    Phase 2 tracker — once a climber is detected, maintains a follow crop
    centered on their last known hip position every frame. Crop size is
    dynamic, based on the last detected bounding box size.
    """
...
class EmergencyZoomTracker:
    """
    Fallback layer — after LOST_FRAMES_TRIGGER missed frames, crops around
    the last known position and retries detection. Runs after Phase 1 and 2.
    """
